This project uses Ant Design (AntD) v6, a React UI component library, with **Phosphor Icons** replacing the default AntD icons, and **i18next** for internationalization. Files in the guidelines directory show how to use AntD components, design tokens, and translations.
...
3. **Use i18n for all text**: Never hardcode user-facing strings. Always use `t('key')` from `useTranslation`.
4. **Use ConfigProvider for theming and locale**: Wrap your app with ConfigProvider to customize theme AND set AntD locale.
5. **Use Semantic DOM styling (v6)**: All components support `classNames` and `styles` props for fine-grained customization of internal DOM elements (e.g., `root`, `header`, `body`, `icon`, `content`). These can be objects or functions.
6. **Prefer controlled components**: Use `value` and `onChange` for form controls.
7. **Use Form for data collection**: Let Form handle form state, not individual component state.
8. **Use `variant` instead of `bordered`**: The `bordered` prop is deprecated in v6. Use `variant="borderless"` instead.
9. **Use `destroyOnHidden` instead of `destroyOnClose`**: The `destroyOnClose` prop is deprecated in v6 for Modal, Drawer, Tabs.
...
```tsx
// WRONG
<Button aria-label="Delete item" icon={<TrashIcon />} />

// CORRECT
<Button aria-label={t('common:a11y.deleteItem')} icon={<TrashIcon weight="bold" />} />
```
