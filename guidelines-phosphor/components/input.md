...
import { UserIcon, InfoIcon, CheckCircleIcon } from './icons';

const { token } = theme.useToken();

<Input
  prefix={<UserIcon weight="regular" />}
  placeholder="Username"
/>

<Input
  suffix={<InfoIcon weight="regular" />}
  placeholder="With suffix icon"
/>

// With validation status icon
<Input
...
// With lock icon prefix
<Input.Password
  prefix={<LockIcon weight="regular" />}
...
<Input.Password
  placeholder="Password"
  iconRender={(visible) => (visible ? <EyeIcon weight="regular" /> : <EyeSlashIcon weight="regular" />)}
/>
```
